...
Now back to `AddressEditor.razor`. Replace **each** of the `<input>` elements with a corresponding `<InputText>` and replace each of the `@bind` attribute names with `@bind-Value` For example,
...
You should prevent accidental double-submission of the form.
...
To check your solution works, you'll want to slow down the server by adding the following line at the top of `PlaceOrder()` inside `OrdersController.cs`:

```cs
await Task.Delay(5000); // Wait 5 seconds
```

Try it at and make sure it all works.
